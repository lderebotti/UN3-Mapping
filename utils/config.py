...
class Config:
    def __init__(self):
        ...
        self.down_sample: bool = (
            True  # apply voxel downsampling to input original point clcoud
        )
        self.vox_down_m: float = (
            0.05  # the voxel size if using voxel downsampling (unit: m)
        )
        ...
        # self.freeze_after_frame: int = (
        #     40  # if the decoder model is not loaded, it would be trained and freezed after such frame number
        # )

        # sampling
        # spilt into 3 parts for sampling: close-to-surface (+ exact beam endpoint) + front-surface-freespace + behind-surface-freespace
        self.surface_sample_range_m: float = (
            0.25  # better to be set according to the noise level (actually as the std for a gaussian distribution)
        )
        self.surface_sample_n: int = 3
        self.free_sample_begin_ratio: float = (
            0.3  # minimum ray distance ratio in front of the surface
        )
        self.free_sample_end_dist_m: float = (
            1.0  # maximum distance behind the surface (unit: m)
        )
        ...
        self.sigma_sigmoid_m: float = (
            0.1  # better to be set according to the noise level (used only for BCE loss as the sigmoid scale factor)
        )
        self.logistic_gaussian_ratio: float = (
            0.55  # the factor ratio for approximize a Gaussian distribution using the derivative of logistic function
        )

        self.proj_correction_on: bool = (
            False  ## conduct projective distance correction based on the sdf gradient or not, True does not work well
        )
        self.loss_weight_on: bool = (
            False  # if True, the weight would be given to the loss, if False, the weight would be used to change the sigmoid's shape
        )
        self.behind_dropoff_on: bool = False  # behind surface drop off weight
        self.dist_weight_on: bool = (
            False  # weight decrease linearly with the measured distance, reflecting the measurement noise
        )
        self.dist_weight_scale: float = 0.8  # weight changing range [0.6, 1.4]

        self.numerical_grad: bool = (
            True  # use numerical SDF gradient as in the paper Neuralangelo for the Ekional regularization during mapping
        )
        self.gradient_decimation: int = (
            10  # use just a part of the points for the ekional loss when using the numerical grad, save computing time
        )
        self.num_grad_step_ratio: float = (
            0.2  # step as a ratio of the nerual point resolution, length = num_grad_step_ratio * voxel_size_m
        )

        self.ekional_loss_on: bool = True  # Ekional regularization (default on)
        self.ekional_add_to: str = (
            "all"  # select from 'all', 'surface', 'freespace', the samples used for Ekional regularization
        )
        self.weight_e: float = 0.5

        self.consistency_loss_on: bool = (
            False  # gradient consistency (smoothness) regularization (default off)
        )
        self.weight_c: float = (
            0.5  # weight for consistency loss, don't mix with the color weight
        )
        self.consistency_count: int = 1000
        self.consistency_range: float = (
            0.05  # the neighborhood points would be randomly select within the radius of consistency_range (unit: m)
        )
        ...
        self.skip_top_voxel: int = (
            2  # slip the top x voxels (mainly for visualization indoor, remove the roof)
        )
        self.mc_mask_on: bool = (
            True  # use mask for marching cubes to avoid the artifacts
        )
    ...
    def load(self, config_file):
        ...
        self.vox_down_m = config_args["process"].get(
            "vox_down_m", self.max_range * 1e-3
        )
        ...
        self.geo_mlp_hidden_dim = config_args["decoder"].get(
            "mlp_hidden_dim", self.geo_mlp_hidden_dim
        )
        ...
        # sampling
        self.surface_sample_range_m = config_args["sampler"].get(
            "surface_sample_range_m", self.vox_down_m * 3.0
        )
        self.free_sample_begin_ratio = config_args["sampler"].get(
            "free_sample_begin_ratio", self.free_sample_begin_ratio
        )
        self.free_sample_end_dist_m = config_args["sampler"].get(
            "free_sample_end_dist_m", self.surface_sample_range_m * 4.0
        )  # this value should be at least 2 times of surface_sample_range_m
        self.surface_sample_n = config_args["sampler"].get(
            "surface_sample_n", self.surface_sample_n
        )
        self.free_front_n = config_args["sampler"].get(
            "free_front_sample_n", self.free_front_n
        )
        self.free_behind_n = config_args["sampler"].get(
            "free_behind_sample_n", self.free_behind_n
        )

        # loss
        self.main_loss_type = config_args["loss"].get("main_loss_type", "bce")
        self.sigma_sigmoid_m = config_args["loss"].get(
            "sigma_sigmoid_m", self.leaf_voxel_size
        )
        self.proj_correction_on = config_args["loss"].get(
            "proj_correction_on", self.proj_correction_on
        )
        self.loss_weight_on = config_args["loss"].get(
            "loss_weight_on", self.loss_weight_on
        )
        if self.loss_weight_on:
            self.dist_weight_scale = config_args["loss"].get(
                "dist_weight_scale", self.dist_weight_scale
            )
            # apply "behind the surface" loss weight drop-off or not
            self.behind_dropoff_on = config_args["loss"].get(
                "behind_dropoff_on", self.behind_dropoff_on
            )
        self.ekional_loss_on = config_args["loss"].get(
            "ekional_loss_on", self.ekional_loss_on
        )  # use ekional loss (norm(gradient) = 1 loss)
        self.weight_e = float(config_args["loss"].get("weight_e", self.weight_e))
        self.numerical_grad = config_args["loss"].get(
            "numerical_grad_on", self.numerical_grad
        )
        if not self.numerical_grad:
            self.gradient_decimation = 1
        else:
            self.gradient_decimation = config_args["loss"].get(
                "grad_decimation", self.gradient_decimation
            )
            self.num_grad_step_ratio = config_args["loss"].get(
                "num_grad_step_ratio", self.num_grad_step_ratio
            )
        self.consistency_loss_on = config_args["loss"].get(
            "consistency_loss_on", self.consistency_loss_on
        )
