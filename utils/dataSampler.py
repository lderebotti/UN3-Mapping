...
class dataSampler():
    ...
    def __init__(self, config: Config):
        ...
        self.all_sample_n = (
            self.config.surface_sample_n 
            + self.config.free_behind_n
            + self.config.free_front_n
            + 1
        )  # 1 as the exact measurement

    def ray_sample(self, origin_torch, points_torch, normal_torch=None, sem_label_torch=None, color_torch=None):
        """
        Sample training sample points for current scan, get the labels for online training
        input and output are all torch tensors
        origin_torch is the sensor's position in coordinate system 
        points_torch is in the sensor's global coordinate system
        """
        ...
        shift_points = points_torch - origin_torch
        distances = torch.linalg.norm(
            shift_points, dim=1, keepdim=True
        ...
        repeated_origin = origin_torch.repeat(all_sample_n, 1)
        repeated_dist = distances.repeat(all_sample_n, 1)
        # print(shift_points.shape, distances.shape)
        # print(repeated_points.shape, all_sample_dist_ratio.shape)
        # print(origin_torch.shape)
        all_sample_points = repeated_points * all_sample_dist_ratio + repeated_origin
        ...
        repeated_points = (
            repeated_points.reshape(all_sample_n, -1, 3)
            .transpose(0, 1)
            .reshape(-1, 3)
        ) # used as rays direction
    ...
    # not used
    def ray_correct_sdf(self, sdf_labels, gradients, rays, tr = 0.3):
        
        sample_num = self.all_sample_n
        grads = gradients[:,:3] #(ray_num, 3)
        grad_uncer = gradients[:, 3]
        # grad_weights = gradients[:,-1]

        sdf_labels = sdf_labels.reshape(-1, sample_num) #(ray_num, sample_num_per_ray)

        #mask = grad_uncer < 0.3
        #print(torch.min(grad_uncer), torch.max(grad_uncer))
        #print(f"valid gradient ratio: {grad_weights[mask].shape[0]/grad_weights.shape[0]}")
        
        # correction
        correct_ratio = torch.abs(F.cosine_similarity(grads, rays)).reshape(-1, 1) #(ray_num, 1)
        #print(correct_ratio.shape)
        #correct_ratio[~mask] = 1.0
        #print(correct_ratio)
        sdf_labels_corrected = sdf_labels * correct_ratio
        #print(sdf_labels_corrected)
        # dsdf = sdf_labels_corrected.abs()-sdf_labels.abs()
        # print(dsdf[dsdf > 0.3])
        # trunction
        sdf_labels_corrected = sdf_labels_corrected.reshape(-1)
        sdf_labels_corrected = torch.clamp(sdf_labels_corrected, min = -tr, max = tr)
        
        return sdf_labels_corrected
    
    # correct sdf labels based on the gradients and rays
    def correct_sdf(self, sdf_labels, gradients, rays, tr = 0.3):
        grads = gradients[:,:3]
        # grad_uncer = gradients[:, 3]
        # grad_weights = gradients[:,-1]
        #mask = (grad_weights > 3) & (sdf_labels.abs() < 2 * tr)
        #mask = (grad_weights > 3) & (sdf_labels.abs() < 2 * tr) & (grad_uncer < 0.3)
        #print(torch.min(grad_uncer), torch.max(grad_uncer))
        #print(f"valid gradient ratio: {grad_weights[mask].shape[0]/grad_weights.shape[0]}")
        # correction
        correct_ratio = torch.abs(F.cosine_similarity(grads, rays))
        #correct_ratio[~mask] = 1.0
        #print(correct_ratio)
        sdf_labels_corrected = sdf_labels * correct_ratio
        # dsdf = sdf_labels_corrected.abs()-sdf_labels.abs()
