...
def batch_mapping(config: Config):
    ...
    run = wandb.init(project="un3-mapping", config=config_dict)

    dataset = DataLoader(config)
    sampler = dataSampler(config)
    feature_field = NeuralVoxelHash(
        config.feature_dim,
    ...
    gradient_field = GradientVoxelHash(
        config.leaf_voxel_size, config.hash_buffer_size, config.device
    )
    ...
    with trange(config.begin_frame, config.end_frame, config.step_frame) as tbar:
        ...
        for frame_idx in tbar:
            ...
            if config.down_sample:
                down_sampled_id = tools.voxel_down_sample_torch(
                    frame_points, config.vox_down_m
                )
            ...
            sample_points, sample_sdfs, sample_normals, _, _, weight, _ = (
                sampler.ray_sample(origin_tensor, frame_points_down, frame_normals_down)
            )
            surface_sample = sample_points[weight > 0]
            feature_field.update(surface_sample)
            gradient_field.update(surface_sample)
            gradient_field.integrate_gradients(
                sample_points, sample_sdfs, sample_normals
            )
    ...
    map_points_o3d.points = o3d.utility.Vector3dVector(
        map_points.detach().cpu().numpy()
    )
    ...
    if config.save_pc_map:
        o3d.io.write_point_cloud(
            config.output_folder + "/merged_map_pc.ply", map_points_o3d
        )
    ...
    opt = optim.Adam(
        [field_param_opt_dict, geo_mlp_param_opt_dict, uncer_mlp_param_opt_dict],
        betas=(0.9, 0.99),
        eps=1e-15,
    )
        ...
        for epoch in range(config.epochs):
            ...
            with tqdm(total=iterations) as iter_pbar:
                for i in range(iterations):
                    ...
                    sample_points, sdf_label, _, _, _, weight, rays = (
                        sampler.ray_sample(
                            iter_start_points, iter_end_points, iter_normals
                        )
                    )
                    ...
                    if config.main_loss_type == "bce":
                        ...
                        sigma, log_sigma = uncer_mlp.uncertainty(features)
                        heter_loss = heter_sdf_loss(
                            sdf_pred, sdf_label, sigma, log_sigma
                        )
                        bce_loss = sdf_bce_loss(
                            sdf_pred,
                            sdf_label,
                            sdf_scale,
                            weight,
    ...
    if config.mesh_recon:
        ...
        artifact = wandb.Artifact(name="reconstructed_mesh", type="mesh")
        artifact.add_file(mesh_filename)
        run.log_artifact(artifact)
        print(f"Successfully saved {mesh_filename} and logged it as a wandb artifact.")
        ...
        if config.main_loss_type == "heter" or config.main_loss_type == "hybrid":
            ...
            map_uncer_color = tools.error2color(
                map_uncer, lower_bound=0, upper_bound=1.0
            )
            map_points_o3d.points = o3d.utility.Vector3dVector(
                map_points.detach().cpu().numpy()
            )
            map_points_o3d.colors = o3d.utility.Vector3dVector(map_uncer_color)
            o3d.io.write_point_cloud(
                config.output_folder + "/merged_map_pc_uncer.ply", map_points_o3d
            )
            print("merged map pointcloud saved.")
            ...
            try:
                ...
                with trange(
                    ...
                    for frame_idx in tbar:
                        ...
                        dynamic_ps.points = o3d.utility.Vector3dVector(
                            dynamic_ps_torch.cpu().numpy()
                        )

                        uncer_ps_torch = frame_points[mask2].detach()
                        uncer_ps = o3d.geometry.PointCloud()
                        uncer_ps.points = o3d.utility.Vector3dVector(
                            dynamic_ps_torch.cpu().numpy()
                        )

                        static_ps_torch = frame_points[~mask].detach()
                        static_ps = o3d.geometry.PointCloud()
                        static_ps.points = o3d.utility.Vector3dVector(
                            static_ps_torch.cpu().numpy()
                        )

                        total_static_ps = torch.cat(
                            (total_static_ps, static_ps_torch), 0
                        )
                ...
                static_map.point["positions"] = o3d.core.Tensor(
                    total_static_ps.cpu().numpy(), dtype, device
                )
                output_path = (
                    config.output_folder
                    + f"/static_output_w1_{w1}_thresh_{threshold}.pcd"
                )
                o3d.t.io.write_point_cloud(
                    output_path, static_map, print_progress=False
                )
